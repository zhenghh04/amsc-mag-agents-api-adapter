...
echo "=== start codex exec-server ==="
codex exec-server --listen "$SANDBOX_URI" >/tmp/execserver.log 2>&1 &
ES_PID=$!; sleep 3
...
python3 tests/test_fidelity.py http://127.0.0.1:8811 /tmp/mag_sbx_marker.txt; RC2=$?

echo; echo "=== shim log (tail) ==="; tail -n 8 /tmp/shim.log
echo; echo "EXIT CODES: spike=$RC1 fidelity=$RC2"
if [ $RC1 -eq 0 ] && [ $RC2 -eq 0 ]; then echo "ALL GREEN"; else echo "SOME FAILED"; fi
